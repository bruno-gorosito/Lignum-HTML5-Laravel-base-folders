<div class="card" data-id="{{$pelicula->id}}">
    <a href={{ route('pelicula.show', ['pelicula' => $pelicula->id]) }}>
        <img src={{ asset('storage/') . '/' . $pelicula->imagen }} alt="Imagen de {{ $pelicula->titulo }}">
        <div class="info">
    ...
    <svg viewBox="0 0 30 30" fill="none" data-id="{{ $pelicula->id }}" xmlns="http://www.w3.org/2000/svg"
        @class(['heart', 'favorito' => $pelicula->favorito])>
        <path
